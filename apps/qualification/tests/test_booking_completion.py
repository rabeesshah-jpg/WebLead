...
def test_build_booking_completion_reply_uses_tts_safe_completion_text():
    ...
    assert result["reply_text"] == TTS_SAFE_COMPLETION
...
@override_settings(BOOKING_LINK=BOOKING_LINK)
def test_finalize_turn_response_uses_tts_safe_completion_without_link_in_reply_text():
    ...
    assert finalized["reply_text"] == TTS_SAFE_COMPLETION
...
def test_completed_extract_response_excludes_booking_link_from_reply_text(mock_extract, client):
    clear_conversations()
