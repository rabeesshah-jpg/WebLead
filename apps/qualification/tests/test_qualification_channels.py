...
@override_settings(N8N_QUALIFICATION_API_SECRET=API_SECRET, BOOKING_LINK=BOOKING_LINK)
@patch("apps.qualification.qualification_turn.extract_qualification_from_openrouter")
def test_text_to_voice_switch_uses_latest_reply_mode(mock_extract, client):
    mock_extract.side_effect = [
        _filter_result(
            accepted_fields={
                "project_type": "new_website",
    ...
    _post_turn(client, message="Facebook", message_sid="SMbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
...
def test_completed_voice_flow_returns_voice_completion_text_and_booking_link(
    ...
    _post_turn(
        client,
        message="I need a new website for my restaurant.",
        message_sid="SMgggggggggggggggggggggggggggggggg",
    )
    _post_turn(client, message="Facebook", message_sid="SMhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh")
    response = _post_turn(
        client,
        input_channel="whatsapp_voice_note",
        media_url=MEDIA_URL,
